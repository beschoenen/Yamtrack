...
class TVStatusTests(TestCase):
    ...
    def test_cascade_skips_excluded_specials(self):
        """Specials do not follow the show when the user excludes them."""
        ...
        statuses = self._season_statuses()
        self.assertEqual(statuses[0], Status.IN_PROGRESS.value)
        self.assertEqual(statuses[1], Status.PAUSED.value)
